...
export class BusinessSectorsEngine {
  // 1. FINTECH & CRYPTO MINING
  public calculateCryptoDailyMetrics(
    rigs: CryptoMiningRig[],
    btcPriceUSD: number,
  ...
  public calculateAIClusterMetrics(
    clusters: AIComputeCluster[],
    powerGridCapacityMW: number = 50
  ): {
  ...
  public calculateRealEstateNOI(
    buildings: CommercialBuilding[],
    market10YRate: number = 0.045
  ): {
  ...
  public calculateFactoryDailyOutput(
    factories: FactoryUnit[],
    marketUnitSellingPrices: Record<string, number>
  ): {
  ...
  public calculateMediaMonthlyRevenue(
  ...
  public calculateGreyMarketDaily(
    operations: GreyMarketOp[],
    currentHeatLevel: number,
    briberyLobbyBudgetMonthly: number
  ): {
